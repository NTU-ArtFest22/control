...
// new activity
  ...
  return {
    ...
    exchange_status: function(ex_data, callback){
      ...
      Activity.find({"_id": mongojs.ObjectId(ex_data.act_id) 
        }, {_id:2, 'group.$':{$elemMatch:{$or:[{"character":ex_data.self_character}, {"character":ex_data.other_character}]}}}, function(err, doc){
        if(err){
